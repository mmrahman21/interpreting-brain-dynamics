'''
This code is used to run downstream experiments in low data regime using only bi-lstm architecture
'''
...
class LSTM(torch.nn.Module):
    ...
    def __init__(self, feature_size, hidden_nodes, output_size, gain):
...
Params_best_gains = {10: {'FBIRN': {'NPT':[0.15, 0.25, 0.5, 0.9, 0.9], 'UFPT': [0.05, 0.05, 0.05, 0.05, 0.05]}, 'ABIDE': {'NPT': [0.4, 0.5, 0.75, 0.75, 0.45, 0.8], 'UFPT': [0.15, 0.2, 0.25, 0.2, 0.35, 0.35]}, 'OASIS': {'NPT': [0.25, 0.35, 0.75, 0.30, 0.4, 0.35], 'UFPT': [0.05, 0.15, 0.3, 0.35, 0.65, 0.65]}}, 20: {'COBRE': {'NPT': [0.25, 0.35, 0.75], 'UFPT': [0.05, 0.45, 0.65]}}}
                     
mode = 2 # int(sys.argv[1])  # Choose FPT (0) / UFPT (1) / NPT (2)
...
samples_per_subject = 13 # Params[Dataset[data]][2]
window_shift = 10 #1
...
# Using just a placeholder for the dir ...NO NPT and UFPT required for the bi-lstm only model
dir = "NPT"   # MODELS[mode]   # NPT or UFPT
wdb = 'wandb'
...
for i in range(len(subjects_per_group)):
    for restart in range(Trials):
        ...
        model = LSTM(X.shape[1], 200, 2, g).float()
        ...
        model.to(device)
        optimizer, accMat[i, restart], aucMat[i, restart] = train_model(model, dataLoaderTrain, dataLoaderTrainCheck, dataLoaderTest, 200, .001)
...
#         dataLoaderFull = get_data_loader(X_sal, Y_sal, X_sal.shape[0])          
        ...
        prefix= f'{Dataset[data]}_spc_{subjects_per_group[i]}_simple_gain_{g}_window_shift_{window_shift}_{dir}_test_id_{test_ID}_bilstm_only'
...
prefix = f'{Dataset[data]}_all_spc_simple_val_best_gains_window_shift_{window_shift}_{dir}_test_id_{test_ID}_bilstm_only'
