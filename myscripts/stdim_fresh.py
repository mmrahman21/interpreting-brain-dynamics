...
class LSTM(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        outputs = [self.decoder(torch.cat((x[0, self.hidden_dim // 2:],
                                           x[-1, :self.hidden_dim // 2]), 0)) for x in lstm_out]
...
def train_model(model, loader_train, loader_test, epochs, learning_rate):
    ...
    for epoch in range(epochs):
        ...
        print("epoch: " + str(epoch) + ", loss: " + str(l.detach().item()) +", test acc: " + str(accuracy / y_test.size(0)) + ", roc: " + str(roc))
...
mode = int(sys.argv[1])  # Choose FPT (0) / UFPT (1) / NPT (2)
data= int(sys.argv[2])  # Choose FBIRN (0)/ COBRE (1) / OASIS (2) / ABIDE (3)
...
samples_per_subject = 7 #13 # Params[Dataset[data]][2]
window_shift = 20 #10 #1
...
# test_indices = [0, 32, 64, 96, 120]  # For FBIRN
# test_indices = [0, 50, 100, 150, 200]   # For ABIDE
# test_indices = [0, 32, 64, 96, 120, 152]  # For OASIS
# test_indices = [0, 16, 32, 48] # For COBRE
...
#         dataLoaderFull = get_data_loader(X_sal, Y_sal, X_sal.shape[0])          
        ...
        prefix= f'{Dataset[data]}_spc_{subjects_per_group[i]}_simple_gain_{g}_window_shift_{window_shift}_{dir}_test_id_{test_ID}_STDIM'
...
prefix = f'{Dataset[data]}_all_spc_simple_val_best_gains_window_shift_{window_shift}_{dir}_test_id_{test_ID}_STDIM'
