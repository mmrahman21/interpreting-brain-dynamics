...
def three_class(samples, t_steps, features, p_steps=10, seed=None):
    if seed != None:
        np.random.seed(seed)
    X = np.zeros([samples, t_steps, features])
    L = np.zeros([samples])
    ...
    for i in range(samples):
        ...
        X[i,:,:] = x
        if label == 0:
            mask[:,0:int(features/3)] = 1
        elif label == 1:
            mask[:,int(features/3):int(features*2/3)] = 1
        else:
            mask[:,int(features*2/3):] = 1
        lift = lift*mask
        X[i,start:start+p_steps, :] += lift
        masks[i,:,:] = lift
        L[i] = int(label)
...
def artificial_batching_patterned_space1(samples, t_steps, features, p_steps=10, seed=None):
    if seed != None:
        np.random.seed(seed)
    X = np.zeros([samples, t_steps, features])
    L = np.zeros([samples])
...
# Data , L, start_positions, masks = artificial_batching_patterned_space2(30000, 140, 50, seed=1988)
# print(Data[0, 0, 0:20])
# print(Data.shape)
# print(L.shape)
