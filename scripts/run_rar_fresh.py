'''
- Multi-fold RAR/ROAR experiments to evaluate saliency methods.
'''
...
for saliency_id in range(8):
    ...
    for i in range(len(subjects_per_group)):
        ...
        prefix= f'{Dataset[data]}_spc_{subjects_per_group[i]}_simple_gain_{g}_window_shift_{window_shift}_{dir}_arch_chk_h_fixed_test_id_{test_ID}_LSTM_milc_prediction'

        for restart in range(Trials):

            filename = os.path.join(basename, prefix+'_'+saliency_options[saliency_id]+'_norm_data_'+str(restart)+'.npy')

            print('Loaded saliency from...', filename)
            sal_map = np.load(filename)
            print(sal_map.shape)


            sal_map = stitch_windows(sal_map, components, samples_per_subject, sample_y, time_points, ws=window_shift)
            print('Average Shape:', sal_map.shape)


        #     It ensures that while creating feature_ranking mask, the zero values will be treated separately 

            epsilon = 0.000000001
            sal_map[sal_map == 0] = sal_map[sal_map == 0] + epsilon
            ...
            # Remove data based on feature ranking and receive the updated data 
                ...
                for t in range(sal_map.shape[0]):
                    Data[t] = compute_RAR_feature_ranking(finalData[t], sal_map[t], Fraction[f])
#                     Data[t] = random_perm_RAR_feature_ranking(finalData[t], sal_map[t], Fraction[f])
            ...
            #     SZ_random = trials_SZ[restart][:samples]
                ...
                accuracy[f, restart], auc[f, restart] = trainer.train(X_train_go, X_test, subjects_per_group[i], restart, run_dir)
        ...
        fprefix = prefix+'_'+name+'_RAR_'+saliency_options[saliency_id]+'_norm_data_zero_fillings_signed_sal_sFNC_eval'
    #     fprefix = prefix+'_'+name+'_RAR'+'_new_RANDOM_perm_small_data_though_same_zero_fillings_sFNC_eval_1'
