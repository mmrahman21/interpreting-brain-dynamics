...
We planned some architectural variations like: using anchoring in the encoder, bidirectional encoder etc. 

Version 1: Anchoring, NO bidirectional encoder, ReLU added
Version 2: NO Anchoring, bidirectional encoder, ReLU added
'''
...
#         prefix= f'{Dataset[data]}_spc_{subjects_per_group[i]}_cross_val_gain_{g}_window_shift_{window_shift}_{dir}_arch_chk_2_test_id_{test_ID}_LSTM_milc'
...
#         model = save_reload_model(model, model_path, prefix, device, restart, save_reload = 'reload')
        ...
        # Compute Saliency
#         saliencies = get_captum_saliency(model, dataLoaderSal, saliency_id, device)
        
#         basename = os.path.join(sbpath, Directories[Dataset[data]], dir, 'Saliency')
#         path1 = os.path.join(basename, prefix+'_chk_prediction'+'_'+saliency_options[saliency_id]+'_'+str(restart))
...
prefix = f'{Dataset[data]}_all_spc_simple_val_best_gains_window_shift_{window_shift}_{dir}_arch_chk_h_fixed_test_id_{test_ID}_LSTM_milc'
